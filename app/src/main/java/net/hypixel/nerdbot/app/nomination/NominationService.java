...
public class NominationService {
    ...
    public NominationSweepReport runMemberNominationSweep() {
        ...
        BiPredicate<Member, DiscordUser> eligibility = (member, discordUser) -> {
            ...
            if (highestRole == null) {
                log.info("Skipping nomination for {} (ID: {}) as they have no roles", member.getEffectiveName(), member.getId());
                return false;
            }

            if (!highestRole.getId().equalsIgnoreCase(memberRoleId)) {
                log.info("Skipping nomination for {} (ID: {}) as their highest role is: {} (ID: {})", member.getEffectiveName(), member.getId(), highestRole.getName(), highestRole.getId());
                return false;
            }
    ...
    public NominationSweepReport runNewMemberNominationSweep() {
        ...
        BiPredicate<Member, DiscordUser> eligibility = (member, discordUser) -> {
            Role highestRole = RoleManager.getHighestRole(member);
            if (highestRole == null) {
                log.info("Skipping new member nomination for {} (ID: {}) as they have no roles", member.getEffectiveName(), member.getId());
                return false;
            }
            ...
            if (joinDate.isAfter(thresholdDate)) {
                log.info("Skipping new member nomination for {} (ID: {}) as they joined on {} (< {} days)", member.getEffectiveName(), member.getId(), joinedAt, minDays);
                return false;
            }
    ...
    private NominationOutcome evaluateNomination(Member member, DiscordUser discordUser, String contextLabel, int requiredVotes, int requiredComments, int requiredMessages, int daysWindow) {
        ...
        if (contextLabel == null) {
            log.info("Checking if {} (ID: {}) should be nominated for promotion (total messages: {}, total comments: {}, total votes: {}, meets messages requirement: {}, meets comments requirement: {}, meets votes requirement: {}, requirements met: {}/3)",
            ...
            log.info("[{}] Checking if {} (ID: {}) should be nominated (total messages: {}, total comments: {}, total votes: {}, meets messages requirement: {}, meets comments requirement: {}, meets votes requirement: {}, requirements met: {}/3)",
    ...
    private void sendNominationMessage(Member member, DiscordUser discordUser, int requiredMessages, int requiredVotes, int requiredComments, int daysWindow) {
        ChannelCache.getTextChannelById(DiscordBotEnvironment.getBot().getConfig().getChannelConfig().getMemberVotingChannelId()).ifPresentOrElse(textChannel -> {
            ...
            log.info("Sent promotion nomination message for {} (ID: {}) in voting channel (total nominations: {})",
