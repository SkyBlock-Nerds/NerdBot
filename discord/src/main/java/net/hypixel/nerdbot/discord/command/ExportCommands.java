...
public class ExportCommands {
    ...
    public void exportMemberActivity(
        ...
        if (inactivityDays == -1 && inactivityMessages == -1) {
            ...
            inactivityDays = inactivityDays != 0 ? inactivityDays : DiscordBotEnvironment.getBot().getConfig().getInactivityDays();
            inactivityMessages = inactivityMessages != 0 ? inactivityMessages : DiscordBotEnvironment.getBot().getConfig().getInactivityMessages();
            ...
            discordUsers.removeIf(discordUser -> {
                ...
                return lastActivity.getChannelActivityHistory().stream()
                    .filter(channelActivityEntry -> !Arrays.asList(DiscordBotEnvironment.getBot().getConfig().getChannelConfig().getBlacklistedChannels()).contains(channelActivityEntry.getChannelId()))
