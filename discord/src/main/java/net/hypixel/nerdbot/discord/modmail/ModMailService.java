package net.hypixel.nerdbot.discord.modmail;
...
public class ModMailService {
    ...
    public static ModMailService getInstance() {
        if (instance == null) {
            synchronized (ModMailService.class) {
                if (instance == null) {
                    ModMailConfig config = BotEnvironment.getBot().getConfig().getModMailConfig();
    ...
    private Optional<User> extractRequesterFromThread(ThreadChannel thread) {
        try {
            ...
            User requester = DiscordBotEnvironment.getBot().getJDA().getUserById(userId);
     ...
     * @param shouldPingRole Whether to append a role mention to the message content based on the configured time between pings
     */
