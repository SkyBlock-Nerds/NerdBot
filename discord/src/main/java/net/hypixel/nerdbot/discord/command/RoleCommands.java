...
public class RoleCommands {
    ...
    public void roleMenu(SlashCommandInteractionEvent event) {
        ...
        PingableRole[] pingableRoles = BotEnvironment.getBot().getConfig().getRoleConfig().getPingableRoles();
            ...
            .thenAccept(user -> {
                ...
                if (isEligibleForPromotion(user)) {
                    ...
                    event.getHook().editOriginal(String.format("You have %s/%s required suggestion votes and %s/%s required suggestion comments to be nominated for the Orange role!",
                        StringUtils.COMMA_SEPARATED_FORMAT.format(user.getLastActivity().getTotalVotes(BotEnvironment.getBot().getConfig().getRoleConfig().getDaysRequiredForVoteHistory())),
                        StringUtils.COMMA_SEPARATED_FORMAT.format(BotEnvironment.getBot().getConfig().getRoleConfig().getMinimumVotesRequiredForPromotion()),
                        StringUtils.COMMA_SEPARATED_FORMAT.format(user.getLastActivity().getTotalComments(BotEnvironment.getBot().getConfig().getRoleConfig().getDaysRequiredForVoteHistory())),
                        StringUtils.COMMA_SEPARATED_FORMAT.format(BotEnvironment.getBot().getConfig().getRoleConfig().getMinimumCommentsRequiredForPromotion())
                    )).queue();
                }
            })
    ...
    private boolean isEligibleForPromotion(DiscordUser user) {
        return user.getLastActivity().getTotalVotes(BotEnvironment.getBot().getConfig().getRoleConfig().getDaysRequiredForVoteHistory()) >= BotEnvironment.getBot().getConfig().getRoleConfig().getMinimumVotesRequiredForPromotion();
    }
