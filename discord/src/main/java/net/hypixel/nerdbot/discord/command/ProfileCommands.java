...
public class ProfileCommands {
    ...
    public static void updateMojangProfile(Member member, MojangProfile mojangProfile) throws HttpException {
        ...
        String newMemberRoleId = BotEnvironment.getBot().getConfig().getRoleConfig().getNewMemberRoleId();
        ...
        if (newMemberRole.isPresent()) {
            if (!RoleManager.hasHigherOrEqualRole(member, newMemberRole.get())) { // Ignore Existing Members
                try {
                    ...
                    String limboRoleId = BotEnvironment.getBot().getConfig().getRoleConfig().getLimboRoleId();
            ...
            .thenCompose(discordUser -> {
                ...
                return requestMojangProfileAsync(event.getMember(), username, true)
                    .thenAccept(mojangProfile -> {
                        ...
                        ChannelCache.getVerifyLogChannel().ifPresentOrElse(textChannel -> textChannel.sendMessage("<@&" + BotEnvironment.getBot().getConfig().getRoleConfig().getModeratorRoleId() + ">").addEmbeds(
                                new EmbedBuilder()
                                    .appendDescription("<@&" + BotEnvironment.getBot().getConfig().getRoleConfig().getModeratorRoleId() + ">")
