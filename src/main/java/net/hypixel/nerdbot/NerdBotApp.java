...
public class NerdBotApp {
    ...
    public static final Cache<String, DiscordUser> USER_CACHE = Caffeine.newBuilder()
            ...
            .removalListener((key, value, cause) -> {
                ...
                getBot().getDatabase().upsertDocument(getBot().getDatabase().getCollection("users", DiscordUser.class), "discordId", discordUser.getDiscordId(), discordUser);
            })
            .build();
    ...
    public static void main(String[] args) {
        ...
        Thread userSavingTask = new Thread(() -> {
            ...
            USER_CACHE.asMap().forEach((s, discordUser) -> {
                log.info("Inserting " + s + " into the users collection!");
