...
public class GeneratorMessages {
    ...
    public static final String DESC_ITEM_NAME = "The name of the generatedItem";
    public static final String DESC_RARITY = "The rarity of the generatedItem";
    public static final String DESC_ITEM_LORE = "The lore of the generatedItem";
    public static final String DESC_TEXT = "The text to display";
    public static final String DESC_TYPE = "The type of the generatedItem";
    public static final String DESC_EXTRA_ITEM_MODIFIERS = "Any modifiers which can be applied to the generatedItem (color, variants, is_player_name)";
    public static final String DESC_DISABLE_RARITY_LINEBREAK = "If you will deal with the line break before the generatedItem's rarity";
    ...
    public static final String DESC_ITEM_ID = "The name of the Minecraft generatedItem you want to display";
    ...
    public static final String DESC_INCLUDE_ITEM = "Include the generatedItem along with the parsed description";
    public static final String DESC_RENDER_INVENTORY = "If the Minecraft Inventory background should be drawn";
    public static final String DESC_RECIPE = "The recipe for crafting the generatedItem (see help recipe for more info)";

    // generatedItem gen general help messages
    public static final String GENERAL_INFO = """
                       ...
                       All of the generatedItem generator commands also have a `hidden` parameter. This can be set to `True` which will only send the image to you so that you can get some practice at making items, or design items in secret! If you do run commands as `hidden`, the image will disappear from chat whenever you restart your Discord client.
                       The generatedItem generation bot is maintained by the Bot Contributors. Feel free to tag them with any issues, or ideas for improvements
                       
                       But most importantly, have fun making items!
                       """.formatted(COMMAND_PREFIX);
    public static final String GENERAL_HELP = """
                       Item Description Generation   ->   `/%1$s help generatedItem`
    ...
    // generatedItem gen generatedItem messages
    ...
    // generatedItem gen generatedItem help messages
    public static final String ITEM_BASIC_INFO = "This is a bot used to create custom items to be used in suggestions. You can use the bot with `/%1$s generatedItem` and `/%1$s full`.".formatted(COMMAND_PREFIX);
    public static final String ITEM_INFO_ARGUMENTS = """
                        `item_name`: The name of the generatedItem. Defaults to the rarity color, unless the rarity is none.
    ...
    public static final String ITEM_INFO_OPTIONAL_ARGUMENTS = """
                        `type`: The type of the generatedItem, such as a Sword or Wand. Can be left blank.
                        `disable_rarity_linebreak (true/false)`: To be used if you want to disable automatically adding the empty line between the generatedItem lore and rarity.
    ...
    public static final String ITEM_COLOR_CODES = """ 
                        ...
                        Another coloring shortcut you can use, such as `%%%%GEM_TOPAZ%%%%`, adding the `[✧]` gemstone slot into the generatedItem.
    ...
    // generatedItem gen head messages
    ...
    // generatedItem gen display generatedItem help messages
    ...
    public static final String DISPLAY_INFO_ARGUMENTS = "`item_id`: The ID of the Minecraft generatedItem that you want to display.";
    public static final String DISPLAY_INFO_OPTIONAL_ARGUMENTS = "`extra_modifiers`: Any extra modifiers for changing how the generatedItem displayed will look (i.e. Hex color for changing leather armor color, skull data)";
    public static final String DISPLAY_INFO_EXTRA_MODIFIERS = """
                       The `extra_modifiers` is where you put any attributes which will change the displayed generatedItem's appearance, each separated by a comma.
    ...
    // generatedItem gen parse messages
    public static final String MISSING_ITEM_NBT = "It seems that you haven't copied the generatedItem's NBT from in game, or is in another format. (Missing NBT Element: `%s`)";
    ...
    // generatedItem gen parse help messages
    public static final String ITEM_PARSE_INFO = "The /%1$s command is extremely useful for being able to copy the NBT of items from in game and convert them into an Item Gen command so that you can modify them.\nUsing this can be extremely helpful for newer users as they will have a base to work from, or easily allows you to modify an existing generatedItem for rewording or upgrades.".formatted(COMMAND_PREFIX);
    public static final String ITEM_PARSE_ARGUMENTS = "`item_nbt`: The Minecraft NBT of the generatedItem (must be copied how you have it in your inventory)";
    public static final String ITEM_PARSE_OPTIONAL_ARGUMENTS = "`include_item (true/false)`: If you want to display the generatedItem along side the parsed image.";

    // generatedItem gen recipe messages
    ...
    public static final String MISSING_FIELD_SEPARATOR = "This Recipe Item (`%s`) seems to not be valid\nIt should follow a similar format to: ```json\n<item_slot_number>,<amount>,<item_name>,<optional extra_details: hex color/enchant>\n```\nEach recipe generatedItem can be separated by %%%%";
    public static final String RECIPE_SLOT_NOT_INTEGER = "You have entered in an generatedItem slot (`%s`) that isn't a number within the recipe generatedItem `%s`";
    public static final String RECIPE_SLOT_DUPLICATED = "It appears that you are trying to put multiple items into one slot (slot number: `%d`, duplicated recipe generatedItem: `%s`).";
    public static final String RECIPE_AMOUNT_NOT_INTEGER = "You have entered in an generatedItem amount (`%s`) that isn't a number within the recipe generatedItem `%s`";
    public static final String RECIPE_AMOUNT_NOT_IN_RANGE = "You have entered an generatedItem amount (`%s`) for the recipe generatedItem `%s` that is not in the range of 1-64. It would be funny if there were negative items required for a recipe though.";

    // generatedItem gen recipe help messages
    ...
    public static final String RECIPE_INFO_ARGUMENTS = """
                               `recipe`: A string containing all of the recipe data for slot number, amount and the generatedItem name, separated by a `%%%%` between different slots.
                               `render_background (true/false)`: If you want to display the Minecraft inventory in the background or leave it transparent.
                               
                               Each recipe generatedItem should be separated with `%%%%`.
                               `slot_number`: The slot you want the generatedItem displayed in (`1-9` starting in the top left).
                               `amount`: The number of that generatedItem you want displayed.
                               `item_id`: The id of the generatedItem with spaces replaced with underscores.
                               `extra_modifiers` (optional): Any extra modifiers that the generatedItem may have.
                               
                               The `item_id` and `extra_modifiers` work the same as displaying an generatedItem through the `/%1$s display` command. Run the `/%1$s help display` to get more information about it!
                               """.formatted(COMMAND_PREFIX);
    ...
    // generatedItem gen full error messages
    public static final String MISSING_FULL_GEN_ITEM = """
                        The parameters you have provided will not be enough to create an generatedItem generation.
                        For each type of generation you want to display, include the following parameters:
                        ```java
                        generatedItem generation         ->  item_name, rarity, item_lore
                        generatedItem render generation  ->  item_id, extra_modifiers
                        recipe generation       ->  recipe
                        ```
                        """;

    // generatedItem gen full help messages
    public static final String FULL_GEN_INFO = """
                        ...
                        Follow the guide for `/%1$s help generatedItem` for more information about creating an generatedItem.
                        Follow the guide for `/%1$s help display` for more information about putting a generatedItem next to the generatedItem lore.
                        Follow the guide for `/%1$s help recipe` for more information about putting a recipe next to the generatedItem lore.
                        """.formatted(COMMAND_PREFIX);
